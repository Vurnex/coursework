...
public class MainGUI extends Application {
		...
		try {
			...
			btn_add.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					...
					if (tf.getText() == "") //my addition
					{//prevents the button from doing anything until a value is entered
	...
	private void createNode()
	{
		...
		boolean insertion_occur = tree.insertNode(node, num_value); //also send the num_value to the method as the key
	...
	{
		int num_value = 0; //my addition
		
		try
		{
			num_value = Integer.parseInt(tf.getText()); //my addition
		}
		catch (NumberFormatException e) //catches the exception and instead throws the following statement
		{
			System.out.println("Please enter a value first");
			return;
		}
		...
		// Insert the node to the tree
		boolean deletion_occur = tree.deleteNode(tree.getSelectedNode(), num_value); //also send the num_vaue as the key
