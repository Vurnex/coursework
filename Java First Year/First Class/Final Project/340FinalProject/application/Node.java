...
public class Node 
{
	...
	public Node deleteRec(Node root, int key) //receives the new node and key from the deleteNode method
	{ //this method constantly recurs down the tree until it finds the value equal to the key
		
		if (root == null)
		{
			return null; //exits the method if the root is not yet set
		}
		
		if (key < root.value) //if key less than current value, go down the left side of the tree
		{
			root.left = deleteRec(root.left, key);
		}
		else if (key > root.value) //if key greater than current value, go down the right side of the tree
		{
			root.right = deleteRec(root.right, key);
		}
		
		else //if the key equals the current value, then this value will be deleted
		{
			//next two if statements will delete node if it has 1 child or none
			if (root.left == null)
			{
				return root.right;
			}
			...
			root.value = minValue(root.right); //if the node has two children, send the value to the minVal method which
												//will get the inorder successor (smallest node in the right subtree)
			
			root.right = deleteRec(root.right, root.value); //delete the successor
		}
		
		
		return root; //returns root to the calling method
	}
	
    int minValue(Node root) //method for determining the inorder successor
    {
        int minVal = root.value; //set minimum value to the current value
        while (root.left != null) //recur down the left side of the tree until it reaches the end
        {
            minVal = root.left.value;
            root = root.left;
        }
        return minVal; //return the minimum value to the calling method which will then be deleted
    }
