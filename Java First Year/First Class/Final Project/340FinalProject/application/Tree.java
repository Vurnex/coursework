...
public class Tree {
	...
	ArrayList<Integer> checkTree = new ArrayList<>(); //list used to check for duplicate integers
	
	
	public boolean insertNode(Node node, int key) //New node / key will be sent by MainGUI.createNode
	{//this method mainly checks for duplicate keys and calls the recursion method to do all the actual work. 

        //Implementation Here...
		
		if (checkTree.contains(key) == true) //checks if the integer exists within the tree already
		{
			System.out.println("That integer is already in the tree.");
			return false; //end the method to prevent further code from being executed
		}
		
		checkTree.add(key); //add the key to the arraylist
		
		root = recurInsert(root, node); //root will equal the output of the first recursion and sends the root value
										//and new node to the recursion method
		
		
		return true; //return true to createNode() for further execution
	}
	
	
	
	public Node recurInsert(Node current, Node newNode) //receives the current node (root if null) and new node from 
														//insertNode()
	{//This method will constantly recur new nodes down the tree where necessary
		
		if (current == null) //set current node to the new node entered 
		{
			current = newNode;
		}
		else if (newNode.getValue() < current.getValue()) //if new node less than current node, apply node to left of 
														//parent
		{
			current.setLeft(recurInsert(current.getLeft(), newNode)); //continue to recur down the tree for each new node
		}
		else if (newNode.getValue() > current.getValue())//if new node greater than current node, apply node to right 
														//of parent
														
		{
			current.setRight(recurInsert(current.getRight(), newNode)); //continue to recur down the tree for each new node
		}
		
		return current; //returns the current node to the calling function
	}
	...
	public boolean deleteNode(Node node, int key) //receives the new node / key from MainGUI.deleteNode()
	{//this method is mainly for checking if node exists in the tree already and sending the new node to the 
		//recursion function
		if (checkTree.contains(key) == false) //checks if key exists in tree before continuing
		{
			System.out.println("That integer does not exist in the tree");
			return false;
		}
		
		checkTree.removeIf(checkTree -> checkTree.equals(key)); //removes key from the arraylist if new key equals any
																//key inside arraylist
		
		node = root; //start at the root
		
		node.deleteRec(node, key); //calls the deleteRec method located in the Node class and sends the new node 
								 //and key to it. 
